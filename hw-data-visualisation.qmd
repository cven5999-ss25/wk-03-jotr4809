...
5.  What are the data types of the three variables used for aesthetic mappings?
    -   x axes has a non-numerical categorical variable that divides the variables into groups, in this case the 5 different continents.

    -   y axe has a numerical continues values for the life expectancy of the population in average.

    -   And the color uses again the data type of the continent which is non-numerical categorical values.
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    -   In this case, geom_bar() does the same graphic. but the difference is that this function makes the height of the bar proportional to the number of cases in each group. In fact is used when we have raw data and want to count occurrences. While geom_col() is used to plot specific data providing the data for the y axes, usually when we have already treated the data and want to be more specific with the shown data.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    -   Both graphics are similar on how they represent the data because they are using rectangles boxes. But geom_histogram() Is actually putting every rectangle just next to the other and representing into a more statistical representation like the gauss bell. Using the frequency of appearance of a variable.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe what you observe here:
    ...
    -   it distributes the data between a range of 40 as a minimum and 85 as a maximum, with a sepparation of 2.5, so that means that would have 18 levels of bars, been the first level from 40 to 42,5 and going on like that adding another 2.5 every level until it gets to 85.
...
5.  Describe in your own words what the `facet_wrap()` function does
    ...
    -   it divides the type of data into different graphics. makes it easier for visualization so we can actually look the behavor of the variable depending on the continent in separated graphics
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`
    ...
    -   Each country has a specific color code associated to its region. So in this case africa has a main color of brow, america its red, Asia is purple, Europe is green and Oceania is blue, and each country has its own specific color tone in that range of color.
